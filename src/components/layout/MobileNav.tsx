...
import {
  ...
  BarChart3, Shield, Users, LogOut, Settings, X,
} from "lucide-react";
...
export function MobileNav({ role, pendingApprovals, open, onClose }: MobileNavProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      {/* Drawer */}
      <aside className="absolute inset-y-0 left-0 flex w-[280px] flex-col bg-[#0a0a0a] shadow-2xl">
        {/* Header */}
        <div className="flex h-14 items-center justify-between border-b border-white/[0.07] px-5">
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-white/40 hover:bg-white/[0.08] hover:text-white transition-colors"
            ...
            {visibleItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    "relative flex items-center justify-between gap-2.5 rounded-lg px-3 py-2.5 text-[13px] font-medium transition-colors",
                    isActive
